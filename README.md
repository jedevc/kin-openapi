...
* `openapi3.Schemas`, `openapi3.ParametersMap`, `openapi3.Headers`, `openapi3.RequestBodies`, `openapi3.ResponseBodies`, `openapi3.SecuritySchemes`, `openapi3.Examples`, `openapi3.Links`, `openapi3.Callbacks`, `openapi3.Encodings` and `openapi3.ServerVariables` are no longer `map[string]*V` type aliases: each is now an alias for the new `openapi3.ComponentMap[E]`, an order-preserving map exposing `Get`/`Value`/`Set`/`Delete`/`Len`/`Keys`/`Iter`/`Map`. Direct map indexing and `range` no longer compile against these types — use `.Get(key)`/`.Value(key)` for lookups and `.Iter()` to range in document order; `.Map()` still returns a plain (unordered) `map[string]*E` for callers that need one.
...
* `openapi3.Responses`, `openapi3.Callback` (the per-operation callback map) and `openapi3.Paths` keep their existing method set unchanged, but gain new `Iter` methods, and `Keys()` now returns keys in document order instead of alphabetical order.
