...
* Document order (as written in the source YAML/JSON) is now preserved throughout loading, validation and marshalling. Previously, decoding through the YAML path lost key order (it round-tripped through an unordered Go map), so any component/property/path ended up in whatever order the old iteration happened to produce.
* `openapi3.Schemas`, `openapi3.ParametersMap`, `openapi3.Headers`, `openapi3.RequestBodies`, `openapi3.ResponseBodies`, `openapi3.SecuritySchemes`, `openapi3.Examples`, `openapi3.Links`, `openapi3.Callbacks`, `openapi3.Encodings` and `openapi3.ServerVariables` are no longer `map[string]*V` type aliases: each is now an alias for the new `openapi3.ComponentMap[E]`, an order-preserving map exposing `Get`/`Value`/`Set`/`Delete`/`Len`/`Keys`/`Iter`/`Map`/`Reorder`. Direct map indexing and `range` no longer compile against these types — use `.Get(key)`/`.Value(key)` for lookups and `.Iter()` to range in document order; `.Map()` still returns a plain (unordered) `map[string]*E` for callers that need one.
* New `openapi3.Webhooks` type (`openapi3.ComponentMap[PathItem]`) replaces the raw `map[string]*PathItem` previously used for `openapi3.T.Webhooks`.
* `openapi3.Content` is likewise no longer a `map[string]*MediaType` alias, but a struct with the same accessors, backed by the new `openapi3.OrderedMap[string, V]`.
* `openapi3.Components`'s `Schemas`, `Parameters`, `Headers`, `RequestBodies`, `Responses`, `SecuritySchemes`, `Examples`, `Links` and `Callbacks` fields are now pointers to the above types (e.g. `*Schemas` instead of `Schemas`).
* `openapi3.Responses`, `openapi3.Callback` (the per-operation callback map) and `openapi3.Paths` keep their existing method set unchanged, but gain new `Iter`/`Reorder` methods, and `Keys()` now returns keys in document order instead of alphabetical order.
